...
def 食物(image,reader):
    背包 = image[-380:200,500:660]  
    总额 = image[-380:200,700:960] 
    # cv2.imwrite('saved_image1.png', 总额)
    results1 = reader.readtext(背包,detail=0) if len(reader.readtext(背包,detail=0)) != 0 else [0]
    results2 = reader.readtext(总额,detail=0) if len(reader.readtext(总额,detail=0)) != 0 else [0]
    背包 = str2value(results1[0])
    总额 = str2value(results2[0])
    角色 = 总额 - 背包
    return [角色,背包,总额]

def 木材(image,reader):
    背包 = image[-325:255,500:660]  
    总额 = image[-325:255,700:960] 
    # cv2.imwrite('saved_image1.png', 总额)
    results1 = reader.readtext(背包,detail=0) if len(reader.readtext(背包,detail=0)) != 0 else [0]
    results2 = reader.readtext(总额,detail=0) if len(reader.readtext(总额,detail=0)) != 0 else [0]
    背包 = str2value(results1[0])
...
def 石材(image,reader):
    背包 = image[-265:315,500:660]  
    总额 = image[-265:315,700:960] 
    # cv2.imwrite('saved_image1.png', 总额)
    results1 = reader.readtext(背包,detail=0) if len(reader.readtext(背包,detail=0)) != 0 else [0]
    results2 = reader.readtext(总额,detail=0) if len(reader.readtext(总额,detail=0)) != 0 else [0]
    背包 = str2value(results1[0])
...
def 金币(image,reader):
    背包 = image[-210:370,500:660] 
    总额 = image[-210:370,700:960] 
    # cv2.imwrite('saved_image1.png', 总额)
    results1 = reader.readtext(背包,detail=0) if len(reader.readtext(背包,detail=0)) != 0 else [0]
    results2 = reader.readtext(总额,detail=0) if len(reader.readtext(总额,detail=0)) != 0 else [0]
    背包 = str2value(results1[0])
...
def get_xlsx(data):
    ...
    for data_list in data:
        ...
        ws3["B"+str(xlsx_index)] = str(int((data_list[1]/ 1000000) * 100) / 100)
        ws3["C"+str(xlsx_index)] = str(int((data_list[2]/ 1000000) * 100) / 100)
        ws3["D"+str(xlsx_index)] = str(int((data_list[3]/ 1000000) * 100) / 100)

        ws["F"+str(xlsx_index)] = data_list[4]  
        ws["G"+str(xlsx_index)] = data_list[5]  
        ws["H"+str(xlsx_index)] = data_list[6]
        ws3["F"+str(xlsx_index)] = str(int((data_list[4]/ 1000000) * 100) / 100)
        ws3["G"+str(xlsx_index)] = str(int((data_list[5]/ 1000000) * 100) / 100) 
        ws3["H"+str(xlsx_index)] = str(int((data_list[6]/ 1000000) * 100) / 100)

        ws["J"+str(xlsx_index)] = data_list[7]  
        ws["K"+str(xlsx_index)] = data_list[8]  
        ws["L"+str(xlsx_index)] = data_list[9]
        ws3["J"+str(xlsx_index)] = str(int((data_list[7]/ 1000000) * 100) / 100) 
        ws3["K"+str(xlsx_index)] = str(int((data_list[8]/ 1000000) * 100) / 100)  
        ws3["L"+str(xlsx_index)] = str(int((data_list[9]/ 1000000) * 100) / 100) 

        ws["N"+str(xlsx_index)] = data_list[10]  
        ws["O"+str(xlsx_index)] = data_list[11]  
        ws["P"+str(xlsx_index)] = data_list[12]   
        ws3["N"+str(xlsx_index)] = str(int((data_list[10]/ 1000000) * 100) / 100)   
        ws3["O"+str(xlsx_index)] = str(int((data_list[11]/ 1000000) * 100) / 100)    
        ws3["P"+str(xlsx_index)] = str(int((data_list[12]/ 1000000) * 100) / 100)      
        xlsx_index+=1     
...
def onLoad_thread():
    ...
    global reader
    reader = easyocr.Reader(['ch_sim'],model_storage_directory=r"\\192.168.6.218\维京崛起\其它\工具版本热更新(非专业勿动勿删)\资源导出\OCR") # 只需要运行一次就可以将模型加载到内存中
    onLoad = True
    print("软件初始化完成！")
    pass
...
def get_rec(leidian_index,list_index):
    ...
    img = str(leidian_index)+".png"
    path = img_path+img
    image = cv2.imread(path)
    模拟器 = [""]+["雷电模拟器-%s"%(leidian_index)]+食物(image,reader)+木材(image,reader)+石材(image,reader)+金币(image,reader)
...
def ss_thread(ld_list):
    for leidian_index in ld_list:
        ...
        run_thread = threading.Thread(target=get_rec, args=[leidian_index,sss])
